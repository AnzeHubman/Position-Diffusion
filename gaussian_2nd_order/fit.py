...
parser.add_argument("--n_mc_steps", type=int, help="No. of MC moves", default=150000)
...
parser.add_argument("--T_hyperparam", type=float, help="Controls acceptance ratio", default=0.1)
...
for block in range(args.n_blocks):
    ...
    d1_D_old, d2_D_old, d3_D_old = gauss.derivatives(y=D_old)
    ...
    mu_old, var_old = gauss.propagators_model(d1_F=d1_F,
                                              d2_F=d2_F,
                                              d3_F=d3_F,
                                              d1_D=d1_D_old,
                                              d2_D=d2_D_old,
    ...
    loss_old = gauss.loss(kl_arr=kl_old)
    ...
    for step in tqdm.tqdm(range(args.n_mc_steps), desc=f"Analyzing block {block + 1}"):
        ...
        d1_D_pro, d2_D_pro, d3_D_pro = gauss.derivatives(y=D_pro)

        # evaluate perturbation
        mu_pro, var_pro = gauss.propagators_model(d1_F=d1_F,
                                                  d2_F=d2_F,
                                                  d3_F=d3_F,
                                                  d1_D=d1_D_pro,
                                                  d2_D=d2_D_pro,
        ...
        loss_pro = gauss.loss(kl_arr=kl_pro)
